...
public class Plane extends Model{
	...
	public float intersectRay(Ray ray) {
		...
		//} else if (testPoint(ray.origin) == Plane.PlaneSide.OnPlane) {
	...
	public Vector2 getUV(Vector3 intersection) {
		//Vector3 planeCenter = new Vector3(normal).scl(d * -1f);
		//Vector3 dInterCen = new Vector3(intersection).sub(planeCenter);
		return uv;
	}
